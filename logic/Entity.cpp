...
double tower_defense::Entity::getSqDistance(const tower_defense::Entity *e) const {
    return pow(e->size/2 + e->getLocation().getX() - this->getLocation().getX(), 2.0f) +
           pow(e->size/2 + e->getLocation().getY() - this->getLocation().getY(), 2.0f) ;
}
...
double tower_defense::Entity::getSqDistanceFromLine(const double A, const double B, const double C)const{
	...
	/// D^2 = (R + x)^2 where we look for x^2
	/// x^2 = (D - R)^2

	/// sorry for using sqrt, math - not even once

	return d - this->size * sqrt(d) + pow(this->size, 2.0f)/4;
}
